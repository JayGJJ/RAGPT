...
class MCR():
    ...
    def _retrieval_vector_generation(self):
        img_name_list = sorted(os.listdir(self.img_path))
        print(f"Encoding images in batches (total: {len(img_name_list)})...")

        images = []
        batch_size = self.batch_size
        for i in tqdm(range(0, len(img_name_list), batch_size), desc="Encoding Image Batches"):
            batch_names = img_name_list[i:i+batch_size]
            batch_images = [Image.open(os.path.join(self.img_path, name)).convert("RGB") for name in batch_names]
...
def print_init_msg(logger, cfg):
    ...
    if "prompt_pos" in cfg.model_para:
        logger.info("Prompt Position: " + f"{cfg.model_para.prompt_pos}")
    if "prompt_type" in cfg.model_para:
        logger.info("Prompt Type: " + f"{cfg.model_para.prompt_type}")
    if "prompt_num" in cfg.model_para:
        logger.info("Number of Prompts: " + f"{cfg.model_para.prompt_num}")
    if "k" in cfg.data_para:
